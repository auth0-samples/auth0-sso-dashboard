...
module.exports = {
  ...
  _requestApps: function(callback) {
    request({
  ...
  _getApps: function(callback, securityTrim) {
    this._requestApps((function(err, apps) {
      if (err) {
        callback(err);
      } else {
        var filtered = [];
  ...
  getAppsForUser: function(callback) {
